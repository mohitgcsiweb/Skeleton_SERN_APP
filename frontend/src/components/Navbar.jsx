      ...
      try {
        if (userData) {
          ...
          const response = await axios.get(
            `${apiUrl}/auth/verify-session/${userId}`,
            {
              headers: {
        ...
        if (error.response && error.response.status === 401) {
          ...
          toast.error(
            error.response.data.message || "Fetching logged in user failed"
          );
        }
      }
    };
    ...
    <>
      {isAuthenticated ? (
        <Navbar
          collapseOnSelect
          expand="lg"
          className="bg-body-tertiary m-0 p-0"
          bg="light"
          data-bs-theme="light"
        >
          <Container>
            ...
            >
              <Nav className="mr-auto">
                <Nav.Link as={Link} to="/" className="topnav-right">
                  <FaHouse size={25} />
                </Nav.Link>
                <NavDropdown
                  title={<FaCircleUser size={25} />}
                ...
                >
                  ...
                  {isAdmin ? (
                    <NavDropdown.Item as={Link} to="/manage-audiences">
                      Manage Audiences
                    </NavDropdown.Item>
                  ) : (
                    ""
                  ...
                  {isAdmin ? (
                    <NavDropdown.Item as={Link} to="/manage-tiles">
                      Manage Tiles
                    </NavDropdown.Item>
                  ) : (
                    ""
